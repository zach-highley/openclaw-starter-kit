...
"Bot (automation)" is a signature inside comments/commits; the GitHub user
...
def load_config(path: Path) -> Config:
    ...
    bot = raw.get("bot", {})
    git_name = bot.get("git_name", "Bot (automation)")
    git_email = bot.get("git_email", "bot@users.noreply.github.com")
    comment_signature = bot.get("comment_signature", "— Bot (automation)")
...
def build_bot_comment(issue: dict, *, kind: str, signature: str) -> str:
    ...
    lines.append("")
    lines.append("_Note: This comment was generated by automation. It is **not** [USER] speaking._")
    if url:
        lines.append(url)
    lines.append("")
    ...
    *,
    ...
    pr_body = "\n".join(
        [
            ...
            "",
            "This change was generated by automation (automation bot).",
...
def main() -> int:
    ...
    for issue in issues:
        ...
        comment = build_bot_comment(issue, kind=comment_kind, signature=cfg.comment_signature)
