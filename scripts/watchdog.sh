#!/bin/bash
# Clawdbot Watchdog v6 — Self-Learning Resilience System
...
CLAWDBOT="clawdbot"                            # Path to clawdbot binary
...
notify() {
    ...
    log "📢 [$urgency] $message"
    run_with_timeout 30 "$CLAWDBOT" message send --channel telegram --message "🤖 Watchdog [$urgency]: $message" >> "$LOG_FILE" 2>&1 || true
}
...
GATEWAY_PID=$(pgrep -f "clawdbot.*gateway" 2>/dev/null | head -1 || echo "")

if [[ -z "$GATEWAY_PID" ]]; then
    log "❌ Gateway not running — starting it"
    FAILURES=$((FAILURES + 1))
    nohup "$CLAWDBOT" gateway >> "$LOG_DIR/gateway-restart.log" 2>&1 &
    sleep 15
    GATEWAY_PID=$(pgrep -f "clawdbot.*gateway" 2>/dev/null | head -1 || echo "")
...
if [[ -n "$GATEWAY_START" ]]; then
    ...
    if [[ $UPTIME_HOURS -gt $MAX_UPTIME_HOURS ]]; then
        ...
        "$CLAWDBOT" gateway stop >> "$LOG_FILE" 2>&1 || true; sleep 5
        nohup "$CLAWDBOT" gateway >> "$LOG_DIR/gateway-restart.log" 2>&1 &
        sleep 15
...
HEALTH_OUTPUT=$(run_with_timeout $HEALTH_TIMEOUT_SEC "$CLAWDBOT" health 2>&1) || HEALTH_OUTPUT="TIMEOUT"
...
if [[ $FAILURES -ge 2 ]]; then
    ...
    "$CLAWDBOT" gateway stop >> "$LOG_FILE" 2>&1 || true; sleep 5
    pkill -f "clawdbot.*gateway" 2>/dev/null || true; sleep 2
    nohup "$CLAWDBOT" gateway >> "$LOG_DIR/gateway-restart.log" 2>&1 &
    sleep 15
    if run_with_timeout 10 "$CLAWDBOT" health 2>&1 | grep -q "Telegram: ok"; then
...
if [[ $FAILURES -ge 4 ]]; then
    ...
    mkdir -p "$ARCHIVE_DIR"
    cp -r "$HOME/.clawdbot/agents/main/sessions/"* "$ARCHIVE_DIR/" 2>/dev/null || true
    rm -f "$HOME/.clawdbot/agents/main/sessions/sessions.json"
    pkill -9 -f "clawdbot.*gateway" 2>/dev/null || true; sleep 3
    nohup "$CLAWDBOT" gateway >> "$LOG_DIR/gateway-restart.log" 2>&1 &
    sleep 15
    FAILURES=0
...
DISK_USED=$(df -h "$HOME/.clawdbot" | tail -1 | awk '{print $5}' | tr -d '%')
